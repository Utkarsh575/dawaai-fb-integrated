      ...
      ScrollView {
        VStack {
          ...
          NavigationLink {
            ...
            HStack {
              VStack {
                Text("Hello \(user.username ?? "Sarthak")").font(.system(size: 25, weight: .semibold)).frame(maxWidth: .infinity, alignment: .center).padding(.leading, 10)
                Text("1 of 3 completed Today").font(.system(size: 15, weight: .light)).frame(maxWidth: .infinity, alignment: .center).padding(.leading, 15)
              }
              Image("heroImage").padding(.horizontal, 20)
            }.frame(maxWidth: .infinity, alignment: .leading).background(Color("heroColor")).cornerRadius(25.0).shadow(color: .green, radius: 2).foregroundColor(.black)
          }
...
//                  print("med dapped", selectedMedicine!)
                }
            }
          }
        }
      ...
      .sheet(isPresented: $showingMedicineInfo) {
        // Ensure selectedMedicine is not nil before passing it to MedicineInfo
...
struct MedicineCta : View {
    ...
    var body: some View {
        HStack{
            ...
            Text("\(medicine.taken)/\(medicine.toBeTaken)").padding(.trailing,30).foregroundColor(.green)
